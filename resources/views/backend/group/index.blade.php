            ...
            <div class="zurb-div-sm-12">
                <div class="hide-bread float-right">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item">客服组管理</li>
                    </ol>
                </div>
                <h4 class="page-title">客服组管理</h4>
            </div>
        </div>
        <div class="new-row zurb-row">
                ...
                <div class="layui-card-body">
                    <div class="row">
                        ...
                        <script type="text/html" id="options">
                            ...
                            @{{#  if(d.group_status == 1){ }}
                            <button class="btn btn-danger waves-effect waves-light btn-xs" lay-event="changeStatus">禁用</button>
                            <a class="layui-table-link" lay-event="changeStatus"></a>
                            @{{#  } else { }}
                            <button class="btn btn-success waves-effect waves-light btn-xs" lay-event="changeStatus">启用</button>
                            @{{#  } }}
                        </script>
                    </div>
                </div>
    ...
    <script>
        layui.use(['layer', 'table'], function(){
            ...
            var dataTable = table.render({
                elem: '#dataTable'
                , height: 500
                , url: "{{url(config('app.backend_route_prefix').'/group/data')}}" //数据接口
                , page: true //开启分页
                , cols: [[ //表头
                    {checkbox: true, fixed: true}
                    , {field: 'id', title: 'ID', sort: true, width: 80, align: 'center'}
                    , {field: 'name', title: '名称'}
                    , {field: 'group_status_format', title: '状态',escape:false}
                    , {field: 'desc', title: '描述'}
                    , {field: 'welcome', title: '问候语'}
            ...
            table.on('tool(dataTable)', function (obj) { //注：tool是工具条事件名，dataTable是table原始容器的属性 lay-filter="对应的值"
                ...
                if (layEvent === 'del') {
                    layer.confirm('确认删除吗？', function (index) {
                        $.post("{{url(config('app.backend_route_prefix').'/group/destroy')}}", {
                            _method: 'delete',
                            ids: [data.id]
                        }, function (result) {
                            if (result.code == 0) {
                    ...
                    layer.open({
                        ...
                        btnAlign: 'c',
                        area: ['630px', '50%'],
                        content: '{{url(config('app.backend_route_prefix').'/group/edit')}}/'+data.id, //这里content是一个DOM，注意：最好该元素要存放在body最外层，否则可能被其它的相对元素所影响
                    ...
                    $.ajax({
                        method: 'GET',
                        url: '{{url(config('app.backend_route_prefix').'/group/change_status')}}/'+data.id,
                        dataType: 'json',
            ...
            $("#create").click(function () {
                layer.open({
                    ...
                    btnAlign: 'c',
                    area: ['630px', '50%'],
                    content: '{{url(config('app.backend_route_prefix').'/group/create')}}', //这里content是一个DOM，注意：最好该元素要存放在body最外层，否则可能被其它的相对元素所影响
                    yes: function (index, layero) {
                        var submit = layero.find('iframe').contents().find('#submitBtn');
                        submit.click(); // 按钮提交表单
                        return false;
